      ...
      <ul class="service__mobile-list">
        <?php foreach ($services as $key => $service) { ?>
          <li class="service__mobile-item <?php if($key === $currentService) { echo 'active'; } ?>">
            <a href="service.php?service=<?=$key?>" class="service__mobile-link <?php if($key === $currentService) { echo 'active'; } ?>"><?= ucfirst($service['title']) ?></a>
          </li>
        <?php } ?>
      </ul>
    </nav>
        ...
        <ul class="service__img-list">
          <?php for ($i = 1; $i <= 3; $i++) { ?>
            <li class="service__img-item">
              <div class="service__img-images <?php if ($i === 1) { echo 'active'; } ?> js-service-images"></div>
              <button class="service__img-icon-wrapper <?php if ($i === 1) { echo 'active'; } ?> js-service-icon">
